...
const eos = new JsonRpc(url, { fetch })
...
function succeeded(res) {
  ...
  if (res.more) {
    if (res.more > lastOne) {
      ...
      eos
        .get_table_by_scope({
          code: CODE, table: tableName, lower_bound: ' ' + lastOne
          , upper_bound: -1, limit: LIMIT
        })
...
eos
  .get_table_by_scope({
    code: CODE, table: tableName, lower_bound: 0, upper_bound: -1
    , limit: LIMIT
  })
  .then(succeeded, failed)
